<style lang="scss">
@import "@/assets/style/project/config.scss";
.AdminService {
    
}
</style>
<template>
    <div class="AdminService o-pt-l">
        <div class="block o-plr-l">
            导出：
            <Button @click="exportList()" :loading="exportLoading" >导出康复记录</Button>
            <Button @click="exportOrganService()" :loading="exportLoading2" style="margin-right:50px">导出康复经费汇总表</Button>

            <Button style="background-color: #6FCDC8 !important;margin-left:60px;">金额：{{fundAmount}}</Button> 
            <!-- <Button style="background-color: #6FCDC8 !important;margin-left:60px;">未报销金额：{{notBxAmount}}</Button>  -->

            <!-- <Button style="background-color: #6FCDC8 !important;">未报销金额：{{costData.isCost}}</Button> 
            <Button style="background-color: #6FCDC8 !important;">已报销金额：{{costData.notCost}}</Button> -->
        </div>
        <div class="block o-plr-l o-mt">
            <span class="o-plr">姓名：</span>
            <el-input v-model="Filter.userName" placeholder="请输入姓名" style="width:10rem;" clearable></el-input>
            <span class="o-plr o-ml">身份证号：</span>
            <el-input v-model="Filter.idCard" placeholder="请输入身份证号" style="width:10rem;" clearable></el-input>
            <span class="o-plr">状态：</span>
            <el-select clearable v-model="Filter.costStatus" placeholder="请选择" style="width:8rem;">
                <el-option v-for="(item,index) in costStausList" :key="item.id" :label="item.name" :value="item.id"></el-option>
            </el-select>
            <span class="o-plr o-ml">用户确认：</span>
             <el-select clearable v-model="useAffirm" multiple collapse-tags placeholder="请选择" style="width:8rem;" @change="changeUseAffirm">
                <el-option v-for="(item,index) in useAffirmList" :key="item.id" :label="item.name" :value="item.id"></el-option>
            </el-select>
            <span class="o-plr o-ml">服务日期：</span>
            <el-date-picker v-model="dateVal" @change="changeDate" class="o-mr" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" style="width:13rem;" value-format="yyyy-MM-dd" :picker-options="DateRangePicker"></el-date-picker>

            
            <Button class="o-ml" @click="MakeFilter();findByCost()">查询</Button>
        </div>
        <div class="block o-plr-l o-mt">
            <el-table class="o-pt" :data="Main.list" v-loading="Main.loading" @selection-change="tableSelection" ref="table">
                <el-table-column type="selection" width="60"></el-table-column>
                <el-table-column prop="id" label="ID" width="70"></el-table-column>
                <el-table-column prop="idCard" label="身份证号" align="center" min-width="100"></el-table-column>
                <el-table-column prop="mobile" label="手机号" align="center"></el-table-column>
                <el-table-column prop="userName" label="姓名" align="center">
                    <template slot-scope="scope">{{ scope.row.userName ? scope.row.userName : '-' }}</template>
                </el-table-column>
                <el-table-column prop="birthday" label="出生日期" align="center"></el-table-column>
                <el-table-column prop="disabilityType" label="残疾类型" align="center"></el-table-column>
                <el-table-column prop="disabilityGrade" label="残疾等级" align="center"></el-table-column>
                <el-table-column prop="amount" label="额度" align="center"></el-table-column>
                <el-table-column prop="useOwnAmount" label="已使用额度" align="center"></el-table-column>
                <!-- <el-table-column prop="serviceItem" label="服务项目" align="center"></el-table-column> -->
                <el-table-column prop="cost" label="服务费用" align="center">
                     <template slot-scope="scope">{{ scope.row.cost ? scope.row.cost + '元' : '-' }}</template>
                </el-table-column>

                
                <el-table-column prop="organName" label="机构名称" align="center"></el-table-column>
                <el-table-column prop="serviceContent" label="服务内容" align="center"></el-table-column>
                <!-- <el-table-column prop="serviceDuration" label="服务时长" min-width="100"></el-table-column> -->
                <el-table-column prop="punchDate" label="打卡日期" min-width="100"></el-table-column>
                
                <el-table-column prop="" label="服务时长" align="center">                    
                    <template slot-scope="scope">{{ scope.row.serviceDuration ? scope.row.serviceDuration + '分钟' : '-' }}</template>
                </el-table-column>
                <el-table-column prop="costStatus" label="状态" min-width="100">
                     <template slot-scope="scope">
                        <span>
                            {{
                                scope.row.costStatus == "Y" ? "已报销":"未报销"
                            }}
                        </span>
                    </template>
                </el-table-column>
                <!-- <el-table-column prop="" label="服务费用" align="center">                    
                    <template slot-scope="scope">{{ scope.row.cost ? scope.row.cost + '元' : '-' }}</template>
                </el-table-column> -->
                <el-table-column prop="" label="用户确认">
                    <template slot-scope="scope">
                        <span>
                            {{
                                 scope.row.useAffirm == "Y" ? "已确认":
                                 (
                                     scope.row.useAffirm == "N"?"已拒绝":
                                     (scope.row.useAffirm == "L"?"待录入":
                                        (scope.row.useAffirm == "D"?"待确认":
                                        (scope.row.useAffirm == "K"?"待离开":'已作废'))
                                     )
                                 )
                            }}
                        </span> 
                    </template>
                </el-table-column>
                <el-table-column prop="remark" label="备注" min-width="100"></el-table-column>
                <el-table-column label="操作" align="center" width="150">
                    <template slot-scope="scope">
                        <div style="display: flex;">
                            <!-- <Button size="small" @click="EditPage(scope.row,'admin/account-id/record/edit')" :plain="!!scope.row.serviceItem"> -->
                            <Button size="small" v-if="scope.row.useAffirm != 'F' && scope.row.useAffirm != 'Y' && scope.row.useAffirm != 'N'"  @click="EditPage(scope.row,'admin/service/edit')" :plain="!!scope.row.serviceItem">
                                <!-- <span v-if="scope.row.useAffirm == 'L' && scope.row.serviceContent">编辑</span> -->
                                <span >录入</span>
                            </Button>
                            <Button size="small" @click="EditPage(scope.row,'admin/service/details')" :plain="!!scope.row.serviceItem">
                                <span >查看</span>
                            </Button>
                            <!-- <Button v-else size="small" @click="EditPage(scope.row,'admin/service/details')" :plain="!!scope.row.serviceItem"> -->

                        </div>
                    </template>
                </el-table-column>
            </el-table>
            <Pagination class="o-mtb" v-model="Page" @turning="Get" :total="Main.total"></Pagination>
        </div>
    </div>
</template>
<script>
import StoreMix from '@/plugins/mixin/store.js'
import DateRange from '@/plugins/mixin/daterange'
export default {
    name: 'AdminService',
    mixins: [StoreMix,DateRange],
    data() {
        return {
            store: 'admin/findByPage',
            useAffirm:[],
            fundAmount:0,
            notBxAmount:0,
            exportLoading:false,
            exportLoading2:false,
            Filter: {
                userName: null,
                pageSize: 16,
            },
            useAffirmList:[
                 {
                    id:"",
                    name:"全部"
                },
                {
                    id:"Y",
                    name:"已确认"
                },
                {
                    id:"N",
                    name:"已否决"
                },
                {
                    id:"D",
                    name:"待确认"
                },
                {
                    id:"L",
                    name:"待录入"
                },
                {
                    id:"F",
                    name:"已作废"
                },
            ],
            costStausList:[
                {
                    id:"",
                    name:"全部"
                },
                {
                    id:"Y",
                    name:"已报销"
                },
                {
                    id:"N",
                    name:"未报销"
                },
                
            ],
            batch: [],
            costData:{},
             dateVal:"",
        }
    },
    computed: {

    },
    methods: {
        changeUseAffirm(){
            if(this.useAffirm.indexOf('')==-1){
                this.Filter.userAffirmStr=this.useAffirm.join()
            }else{
                this.Filter.useAffirm=''
            }
        },
         changeDate(){
            if(this.dateVal){
                this.Filter.serviceDateGE  = this.dateVal[0]
                this.Filter.serviceDateLE = this.dateVal[1]
            }else{
                this.Filter.serviceDateGE  = ''
                this.Filter.serviceDateLE = ''
            }
        },
        findByCost(){
            this.Dp('main/FIND_AMOUNT',this.Filter).then(data=>{
                if(data.code == '200'){
                    this.fundAmount = data.data.bussData.fundAmount
                    this.notBxAmount = data.data.bussData.notBxAmount

                    // this.Target.expAmount = data.data.bussData.isCost
                    // this.Target.noAmount = data.data.bussData.notCost
                }
            })
        },
        init(){
            this.reload()
        },
        reload(){
            
          let page =  1
            
            if(JSON.parse(window.localStorage.getItem("filterData"))&&JSON.parse(window.localStorage.getItem("filterData")).organId==this.User.id){
                page =  Number(window.localStorage.getItem('backPage'))
                this.Filter = JSON.parse(window.localStorage.getItem("filterData"))
            }else{
                page =  1
                this.Filter = { pageSize: 16, userName: null,}
            }
            this.Filter.organId = this.User.id
            this.Filter.userId=null

            this.Page=page
            this.Get(page)
            // this.Get()
            this.findByCost()
        },
        tableSelection(list=[]){
            this.$set(this,'batch',[])
            for(let item of list){
                this.batch.push(item.id)
            }
        },
        exportOrganService(){
            // var data = {
            //     userNameLike:this.Filter.userName,
            //     idCard:this.Filter.idCard,
            //     organId:this.Filter.organId,
            //     serviceDateGE:this.Filter.serviceDateGE,
            //     serviceDateLE:this.Filter.serviceDateLE
            // }
            // console.log(this.Filter.serviceDateLE)
            //if(!this.Filter.serviceDateGE||!this.Filter.serviceDateLE){
            //    this.Err('请选择导出时间,时间不超过90天')
            //    return
            //}
            //let start  = new Date(this.Filter.serviceDateGE.replace(/-/g,"/")).getTime();
            //let end = new Date(this.Filter.serviceDateLE.replace(/-/g,"/")).getTime();
            //if(end - start  > 93*24*60*60*1000){
            //    this.Err('导出时间范围超过90天,请分段导出')
            //    return
            //}
            
            var data=this.Filter
            Object.assign(data,{ ids: this.batch,loginId:this.User.id })
            this.exportLoading2=true

            this.Dp('admin/EXPORTS_ORGAN_SERVICE',data).then(buffer=>{
                if(buffer){
                    this.SaveBufferFile(buffer,'导出康复经费汇总表.pdf','pdf')
                }
            this.exportLoading2=false

            }).catch(err=>{
                this.exportLoading2=false
                // console.log(data.toString())
                // let msg= String.fromCharCode.apply(null, new Uint8Array(err));
                // let data = JSON.parse(msg)
                // this.Err(data.data.errMsg)
                this.Err('导出失败')
            })
        },
        exportList(){
            var data = {}
            if(!this.Filter.serviceDateGE||!this.Filter.serviceDateLE){
                this.Err('请选择导出时间,时间不超过30天')
                return
            }
            let start  = new Date(this.Filter.serviceDateGE.replace(/-/g,"/")).getTime();
            let end = new Date(this.Filter.serviceDateLE.replace(/-/g,"/")).getTime();
            if(end - start  > 31*24*60*60*1000){
                this.Err('导出时间范围超过30天,请分段导出')
                return
            }
            Object.assign(data,this.Filter,{ ids: this.batch,loginId:this.User.id })
            this.exportLoading=true
            this.Dp('admin/EXPORTS_SERVICE',data).then(buffer=>{
                if(buffer){

                    this.SaveBufferFile(buffer,'导出康复记录.pdf','pdf')
                }
            this.exportLoading=false

            }).catch(err=>{
                this.exportLoading=false
                this.Err('导出失败')

            })
        },
    },
    components: {

    },
    mounted(){        
        this.Filter.organId = this.User.id
        this.init()
    },
    created() {

    },
    activated(){
        this.init()
    },
}
</script>